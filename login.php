						...
						<?php
							...
							$conn= mysqli_connect($server_name, $username, $password, $database_name, $port);
								...
								if(isset($_POST['login']))
								{
									...
									$sql_query2 = "SELECT user_email, user_password from user_credentials WHERE username = '$email';";
									$result = mysqli_query($conn, $sql_query2);
										while($row = $result->fetch_assoc()){
										$hush = $row['user_password'];
										echo '<script>alert("'.$hush.'")</script>';
										if($result-> num_rows > 0){
											

											if(password_verify($pass, $hush))
											{
										//echo "Naa";
												...
												echo '<script>alert("Error! Wrong Password. '.$hush.'")</script>';
											}
										}	
										else
										{
